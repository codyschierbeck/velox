...
std::shared_ptr<facebook::velox::FlatVector<facebook::velox::StringView>> convertOutput(const std::vector<std::string>& output, size_t repeatCount) {
    std::vector<std::optional<facebook::velox::StringView>> repeatedOutput(output.size() * repeatCount);

    using StringView = facebook::velox::StringView;

    for (size_t i = 0; i < repeatCount; ++i) {
        for (size_t j = 0; j < output.size(); ++j) {
           repeatedOutput[i * output.size() + j] = !output[j].empty() ? std::optional<StringView>(output[j]) : std::nullopt;
  ...
  std::shared_ptr<facebook::velox::SimpleVector<facebook::velox::StringView>> testingRegexReplaceRowsSimple(
      const std::vector<std::optional<std::string>>& input,
  ...
  std::shared_ptr<facebook::velox::SimpleVector<facebook::velox::StringView>> testingRegexReplaceSimpleConstantPattern(
      const std::vector<std::optional<std::string>>& input,
      std::string pattern,
      const std::vector<std::optional<std::string>>& replace,
...
TEST_F(RegexFunctionsTest, RegexReplaceNegativePosition) {
  EXPECT_THROW(
      testRegexReplaceSimpleOnce({"abc"}, "a", "",  {-1}),
      VeloxRuntimeError);
}

TEST_F(RegexFunctionsTest, RegexReplaceZeroPosition) {
  EXPECT_THROW(
      testRegexReplaceSimpleOnce({"abc"}, "a", "",  {0}),
      VeloxRuntimeError);
}
...
TEST_F(RegexFunctionsTest, RegexReplaceConstantPattern) {
  ...
  const std::vector<std::string> outputVector = {"the sky was blue", "coding isn't fun"};
...
TEST_F(RegexFunctionsTest, RegexReplaceDataframeMultiple) {
  ...
  const std::vector<std::string> outputVector = {"fruit fruit fruit", "fruit fruit fruit"};
  auto result = testingRegexReplaceRowsSimple(
...
TEST_F(RegexFunctionsTest, RegexReplaceDataframeOffsetPosition) {
  ...
  const std::vector<std::string> outputVector = {"apple pie fruit", "grape fruit grape"};
  auto result = testingRegexReplaceRowsSimple(
